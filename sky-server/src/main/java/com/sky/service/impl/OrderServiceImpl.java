...
public class OrderServiceImpl implements OrderService {
     ...
     * @return
     */
    public PageResult pageQuert4User(int pageNum, int pageSize,Integer status) {
        // 设置分页
    ...
    public void repetition(Long id) {
        ...
        {
            ShoppingCart shoppingCart = new ShoppingCart();
            // 将原订单详情里面的菜品信息重新复制到购物车对象中,除了id其他都复制
    ...
    private List<OrderVO> getOrderVOList(Page<Orders> page){
        ...
        if (!CollectionUtils.isEmpty(ordersList)){
            for (Orders orders : ordersList){
                OrderVO orderVO = new OrderVO();
                BeanUtils.copyProperties(orders,orderVO);
                String orderDishes = getOrderDishsStr(orders);
    ...
    private String getOrderDishsStr(Orders orders){
        // 根据订单ID查询该订单包含的所有菜品详情（如菜品名、数量）
        List<OrderDetail> orderDetailList = orderDetailMapper.getByOrderId(orders.getId());
        // 用Stream流将每个菜品详情转换为"菜品名*数量;"格式的字符串
        List<String> orderDishList = orderDetailList.stream()
                .map(x -> x.getName() + "*" + x.getNumber() + ";")
                .collect(Collectors.toList());
        // 将所有菜品字符串拼接成一个完整字符串（如"宫保鸡丁*3;鱼香肉丝*2;"）
